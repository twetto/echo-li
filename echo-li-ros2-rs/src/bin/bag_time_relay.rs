...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // A megapixel mono image is about 1 MB, which the middleware splits into
    // hundreds of loopback UDP datagrams. Best effort loses the whole frame to a
    // single missing fragment and never asks for it again: a third of the frames
    // went that way, on this relay and on an rclpy one alike. Reliable recovers
    // them.
