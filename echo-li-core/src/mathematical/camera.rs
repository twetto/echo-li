...
impl CameraModel for PinholeModel {
    fn project(&self, p: &Vector3<f64>) -> Vector2<f64> {
        Vector2::new(self.fx * p[0] / p[2] + self.cx, self.fy * p[1] / p[2] + self.cy)
    }
    fn undistort(&self, uv: &Vector2<f64>) -> Vector3<f64> {
        Vector3::new((uv[0] - self.cx) / self.fx, (uv[1] - self.cy) / self.fy, 1.0).normalize()
    }
    fn projection_jacobian(&self, p: &Vector3<f64>) -> Matrix2x3<f64> {
        let z_inv = 1.0 / p[2];
        let z_inv2 = z_inv * z_inv;
        Matrix2x3::new(
            self.fx * z_inv, 0.0, -self.fx * p[0] * z_inv2,
            0.0, self.fy * z_inv, -self.fy * p[1] * z_inv2,
        )
    }
}
...
pub struct RadTanModel {
    pub fx: f64, pub fy: f64, pub cx: f64, pub cy: f64,
    pub k1: f64, pub k2: f64, pub p1: f64, pub p2: f64,
}
...
impl CameraModel for RadTanModel {
    ...
    fn projection_jacobian(&self, p: &Vector3<f64>) -> Matrix2x3<f64> {
        ...
        Matrix2x3::new(
            self.fx * dxd_dx * z_inv, self.fx * dxd_dy * z_inv, self.fx * (-dxd_dx * x * z_inv - dxd_dy * y * z_inv),
            self.fy * dyd_dx * z_inv, self.fy * dyd_dy * z_inv, self.fy * (-dyd_dx * x * z_inv - dyd_dy * y * z_inv),
        )
    }
}
