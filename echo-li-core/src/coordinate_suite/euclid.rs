use nalgebra::{DMatrix, DVector, Matrix3, Vector2, Vector3, Matrix2x3};
use echo_lie::{SO3, SE3, SOT3, base::LieGroup};

use crate::mathematical::vio_state::{VIOState, VIOSensorState, GRAVITY_CONSTANT};
...
impl EqFCoordinateSuite for EuclideanSuite {
    fn state_chart(&self, xi: &VIOState, xi0: &VIOState) -> DVector<f64> {
        ...
        for i in 0..n {
            eps.fixed_rows_mut::<3>(s + 3 * i).copy_from(&(xi.camera_landmarks[i].p - xi0.camera_landmarks[i].p));
        }
    ...
    fn state_matrix_a(&self, x: &VIOGroup, xi0: &VIOState, imu_vel: &IMUVelocity) -> DMatrix<f64> {
        ...
        // Pose pos -> vel
        a0t.fixed_view_mut::<3, 3>(9, 12).copy_from(&Matrix3::identity());
        // Vel -> Orientation (Gravity)
        a0t.fixed_view_mut::<3, 3>(12, 6).copy_from(&(-GRAVITY_CONSTANT * base_skew(&xi0.sensor.gravity_dir())));
        ...
        a0t.fixed_view_mut::<6, 6>(15, 15).copy_from(&SE3::adjoint_algebra(&transformed));
    ...
    fn input_matrix_b(&self, x: &VIOGroup, xi0: &VIOState) -> DMatrix<f64> {
        ...
        bt.fixed_view_mut::<6, 6>(0, 6).copy_from(&nalgebra::SMatrix::<f64, 6, 6>::identity());
        bt.fixed_view_mut::<3, 3>(6, 0).copy_from(&r_a);
        bt.fixed_view_mut::<3, 3>(9, 0).copy_from(&(base_skew(&x.a.translation) * r_a));
        bt.fixed_view_mut::<3, 3>(12, 0).copy_from(&(r_a * base_skew(&xi_hat.sensor.velocity)));
    ...
    fn output_matrix_ci_star(&self, q0: &Vector3<f64>, q_hat: &SOT3, cam: &dyn CameraModel, y: &Vector2<f64>) -> Matrix2x3<f64> {
        ...
        m2g.fixed_view_mut::<3, 3>(0, 0).copy_from(&(-base_skew(q0) / qq));
        m2g.fixed_view_mut::<1, 3>(3, 0).copy_from(&(-q0.transpose() / qq));

        let d_rho = |y_vec: &Vector3<f64>| {
            let mut d_rho_vec = nalgebra::Matrix3x4::<f64>::zeros();
            d_rho_vec.fixed_view_mut::<3, 3>(0, 0).copy_from(&base_skew(y_vec));
    ...
    fn lift_innovation(&self, total_innovation: &DVector<f64>, xi0: &VIOState) -> VIOAlgebra {
        ...
        for i in 0..n {
            ...
            wi.fixed_rows_mut::<3>(0).copy_from(&(-q0.cross(&gamma_qi) / qq));
