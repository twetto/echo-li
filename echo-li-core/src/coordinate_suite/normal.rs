...
fn sensor_chart_inv_normal(
    eps: &nalgebra::SVector<f64, 21>,
    xi0: &VIOSensorState,
) -> VIOSensorState {
    ...
    xi.velocity = xi
        ...
        .rotation
        .inverse()
        .act(&(v_xi0_world + xi0.pose.rotation.act(&v_a)));
    xi
}
...
impl EqFCoordinateSuite for NormalSuite {
    ...
    fn state_matrix_a(&self, x: &VIOGroup, xi0: &VIOState, imu_vel: &IMUVelocity) -> DMatrix<f64> {
        ...
        for i in 0..n {
            ...
            a_normal
                .fixed_view_mut::<3, 3>(s + 3 * i, s + 3 * i)
                .copy_from(&new_li_li);
        }
    ...
    fn propagation_blocks(
        ...
        let euc = EuclideanSuite.propagation_blocks(x, xi0, imu_vel);
        let n = xi0.camera_landmarks.len();
        let s = VIOSensorState::CDIM;
        ...
        for i in 0..n {
            ...
            let new_li_s = m_e2n * euc.a_lm_s.fixed_view::<3, 21>(row, 0) * m_s_inv;
            a_lm_s.fixed_view_mut::<3, 21>(row, 0).copy_from(&new_li_s);

            a_lm_lm.push(m_e2n * euc.a_lm_lm[i] * m_n2e);

            let new_b_li = m_e2n * euc.b_lm.fixed_view::<3, 12>(row, 0);
            b_lm.fixed_view_mut::<3, 12>(row, 0).copy_from(&new_b_li);
        }
    ...
    fn lift_innovation(&self, total_innovation: &DVector<f64>, xi0: &VIOState) -> VIOAlgebra {
        ...
        for i in 0..n {
            ...
            inn_euc
                .fixed_rows_mut::<3>(s + 3 * i)
                .copy_from(&inn_euc_lm);
        }
