...
use echo_lie::{SE3, SO3, SOT3, base::LieGroup};
...
pub fn state_distance(xi1: &VIOState, xi2: &VIOState) -> f64 {
    ...
    sum_sq += xi2
        ...
        .camera_offset
        .inverse()
        .compose(&xi1.sensor.camera_offset)
        .log()
        .norm_squared();
...
pub fn random_state_element<R: Rng>(n_landmarks: usize, rng: &mut R) -> VIOState {
    ...
    VIOState {
        sensor: VIOSensorState {
            ...
            pose: SE3::new(
                SO3::exp(&Vector3::new(rng.random(), rng.random(), rng.random())),
...
pub fn reasonable_sensor_state<R: Rng>(rng: &mut R) -> VIOSensorState {
    ...
    VIOSensorState {
        ...
        pose: SE3::new(
            SO3::exp(&(Vector3::from_fn(|_, _| normal.sample(rng)) * 0.2)),
...
pub fn reasonable_group_element<R: Rng>(n_landmarks: usize, rng: &mut R) -> VIOGroup {
    ...
    for i in 0..n_landmarks {
        q.push(SOT3::new(
            SO3::exp(&(Vector3::from_fn(|_, _| normal.sample(rng)) * 0.1)),
            (normal.sample(rng) * 0.1).exp(),
        ));
        id.push(i as u64);
    }
    ...
    VIOGroup {
        ...
        a: SE3::new(
            SO3::exp(&(Vector3::from_fn(|_, _| normal.sample(rng)) * 0.1)),
            Vector3::from_fn(|_, _| normal.sample(rng)) * 0.1,
        ),
...
pub fn random_velocity_element<R: Rng>(
    rng: &mut R,
) -> crate::mathematical::imu_velocity::IMUVelocity {
    let normal = Normal::new(0.0, 1.0).unwrap();
